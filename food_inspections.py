...
def search_by_location(dataframe): 
    '''The purpose of the search_by_location function is to find all restaurants within 0.5 mi of where Dr. Romano lives that failed an inspection since 11/1/16
        It receives a DataFrame as an argument.  This DataFrame contains all of the original data from Food_Inspections.csv
        The return value is a DataFrame with rows matching the aforementioned description sorted by increasing distance from where Dr. Romano lives'''
    
    def dcalc(so):#function passed to DataFrame.apply that calculates distance from where Dr. Romano lives
    
        r = 3961
        la1 = 0.7310728810371 #Dr. Romano's latitude in radians
        lg1 = -1.52971051 #Dr. Romano's longitude in radians
